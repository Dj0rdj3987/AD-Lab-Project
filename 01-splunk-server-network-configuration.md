...
This is the first part of a home lab build documented in this repository: a small Active Directory environment made up of four VirtualBox VMs  an Ubuntu server running Splunk (the SIEM), a Windows 10 target machine, a Windows Server domain controller, and a Kali Linux box as the attacker , built so I could join a real user to a real domain, collect that machine's telemetry in Splunk, then attack it from Kali (starting with a brute-force run, with Atomic Red Team next) and go find the attack in the telemetry afterward. (See the [README](README.md) for the full architecture)

I built it in the order these parts are numbered, and that's the order this write up follows too. The Splunk server comes first for a practical reason: the Universal Forwarder on the target machine needs a fixed address to send its data to, so the SIEM had to be sitting at a stable IP before I could point anything at it , nothing else in the lab actually depends on Splunk beyond that. All four machines just sit on the same lab network, the target machine is the one that joins the AD domain, later on in Part 8, and Splunk's job the whole time is simply to receive and hold onto whatever telemetry gets forwarded to it. That's what this first part is about.
